...
var COLOR = { shift:"#1f4e79", assign:"#e8730c" }; // ネイビー / オレンジ
// =========================
...
// 1行ぶん（ラベル＋レーン＋複数バー）を描く。barsは{start,end,name}の配列
function laneHTML(label, bars, color){
  ...
  bars.forEach(function(b){
    ...
    html += '<div title="'+(b.name||"")+' ('+t+')" style="position:absolute;left:'+left+'%;width:'+width+'%;top:4px;bottom:4px;background:'+color+';color:#fff;border-radius:4px;font-size:11px;display:flex;align-items:center;justify-content:center;overflow:hidden;white-space:nowrap;padding:0 4px">'+t+'</div>';
  });
  return html+'</div></div>';
}
...
function draw(){
  ...
  order.forEach(function(nm){ html += laneHTML(nm, rows[nm], COLOR.shift); });

  // ---- 案件（1行に横並び）----
  if(assigns.length>0){
    var bars = assigns.map(function(a){ return {start:a[ASSIGN.start], end:a[ASSIGN.end], name:a[ASSIGN.name]}; });
    html += laneHTML("案件", bars, COLOR.assign);
  }
